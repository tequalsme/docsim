...
// TODO enhance to allow passing of additional configuration params as sys props (num tokens, match sizes, etc.)
public class SimilarityExec {
    ...
    private static void evaluate(String fileName1, String fileName2) {
        try {
            ...
            System.out.println(String.format("Files '%s' and '%s' are%s similar, as evaluated by %s", fileName1,
                    fileName2, (areSimilar ? "" : " not"), similarity.getClass().getName()));

        } catch (Exception ex) {
            System.err.println("An unexected error occurred: " + ex);
        }
    }

    @SuppressWarnings("unchecked")
    private static Similarity getImplementaion() throws InstantiationException, IllegalAccessException,
